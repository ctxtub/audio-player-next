...
完成清理后复用并更新最少数量的现有场景，不机械新增文件，将验收收敛为以下四条完整旅程：

- 在同一会话依次创作两篇故事：AI 作品集标题在创作页出现，两个作品卡都可原位准备、播放、暂停和继续；进入故事库后只出现一个作品集，详情包含两条作品。
- 保持连续创作默认开启：当前作品播放时页面展示下一篇生成、保存、语音准备和 ready；下一篇晚于当前作品结束时显示 waiting，准备完成后自动播放，等待期间预算不减少，并最终在预算耗尽时停止。
- 在播放和下一篇准备期间点击“新建创作”：声音与全局播放器立即停止，旧卡片、下一篇、错误和进度全部清空，页面回到新作品集初始态，迟到结果不复活。
- 从故事库作品集详情播放任意作品：Mini / Expanded 与当前 Work、作品集标题和秒级进度一致；刷新及跨页面后合理续播，列表底部在 Mini Player、TabBar 和设备安全区之上完整可见。

每条旅程只用可见按钮、文案、高亮、时间和进度作为通过依据。登录 fixture、上游 mock 和隔离存储仅提供安全环境，不得替用户完成旅程步骤。
