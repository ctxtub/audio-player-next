#!/usr/bin/env node
/**
 * 中文注释：actions pin 真实性校验器（供应链硬化，2026-09-11）。
 *
 * 背景：2026-09-10 的 supply-chain-pin 扫描把 5 个 action 钉到了**编造的 SHA** 上
 * （GitHub 返回 "No commit found for SHA"），导致 4 个 workflow 的每个 job
 * 都在 "Set up job" 秒挂。旧守卫只做形状校验 /@[0-9a-f]{40}/，
 * **形状正确但值不存在的 SHA 能完美通过**——这是纸老虎。
 *
 * 本脚本做真实解析：取 workflow 里 `uses: owner/repo@<sha> # <tag>` 的每一处，
 * 向 GitHub API 解析该 tag 的真实 commit SHA，与钉住值逐字比对。
 *
 * 用法：
