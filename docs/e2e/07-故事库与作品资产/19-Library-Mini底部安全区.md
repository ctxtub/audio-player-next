...
2. **有 Mini 抬升（docked 路径，窄视口 767px）**：播放任意作品使 Mini 出现（`mini-slot[data-visible="true"]` 且 `main-chrome[data-has-docked-mini="true"]`），读取四变量计算值并断言数值组合成立：`--bottom-chrome-safe-bottom ≈ --main-tabbar-occupied-height + --mini-player-occupied-height + --bottom-chrome-gap`（±1px），其中 Mini 占用 > 0。
