...
/**
 * 主导航路由契约与页面旅程（产品覆盖层真场景）。
 *
 * 覆盖矩阵（现代化：/player 旧 Player 页面契约已随  退役，冻结为 /library redirect）：
 ...
 * 2. Compatibility Redirect Journey（冻结）：
 ...
 *      /player redirect 与 Browser Back 为整页文档导航，只锁共享布局单宿主挂载（MPA 语义与 /D 一致）
 * 4. 显式前置处理新手引导弹窗（chat_onboarding_seen_v1），确保无偶发遮挡。
 */
test("主导航路由契约与页面旅程", async ({ page, harnessEnv }) => {
    ...
    // 9. Compatibility redirect journey（冻结）：直接进入 /player → 最终落到 /library
