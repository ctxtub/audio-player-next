...
2. **方案**：非平凡变更先写清方案与取舍；实施完成后只保留仍约束当前产品或架构的内容。
3. **计划**：把工作拆成可独立验证的实施步骤；临时计划不作为长期仓库资产。
...
- 行为契约变化：更新当前产品契约和可见验收标准；优先复用完整用户旅程，不机械新增多层测试。
...
- 已完成方案不按轮次堆积；仍有解释价值的决策压缩到 `docs/archive/**`，无长期价值的计划和过程文档直接删除。
- 原始日志、截图、数据库和 transcript 不入 Git。
