...
/**
 *  NowPlayingActions（spec §33-§34 Expanded 内容动作区）。
 * +  Draft Transcript 入口（spec §35 additive，不破 Work 面）。
 * +  Creation Actions Boundary（spec §36/§37 additive，不破既有口）。
 *
 ...
 * - Work「继续创作」（spec §36.2/）： `continueFromStoryWork`
...
export type NowPlayingActionsProps = {
    ...
    onViewStory: () => void;
    /**
     *  Draft 正文（ Session.storyText 原文；缺省空即 Draft 口隐藏，
     * 保持  调用兼容）。
     */
    storyText?: string | null;
    /**
     *  Draft 会话状态（缺省 paused，保持旧调用兼容；idle 即隐藏）。
     * 显式传入时与 ViewModel.canViewTranscript 同口径（fail-closed）。
     */
    status?: PlaybackSessionStatus;
    /**
     *  查看正文回调（Draft 口，父级：只切 Expanded 局部 view →
     * TranscriptView，不导航、不改 Session）。
     */
    onViewTranscript?: () => void;
    /**
     *  返回创作回调（Draft 口，spec §37，父级：closeExpanded 先行
     * 再 push('/chat')，绝不自动发送新 Prompt；缺省/undefined 时该口隐藏）。
     */
    onBackToCreation?: () => void;
    /**
     *  继续创作回调（Work 口，spec §36.2/ additive-ready 预留：
     *  `continueFromStoryWork` 契约落地后由父级委托该契约；本轮判定恒
...
export const NowPlayingActions: React.FC<NowPlayingActionsProps> = ({
    ...
    //  Work 口冻结（行为不变：合法 Work 目标即展示，导航口）。
    //  Work 继续创作 additive-ready：判定恒隐藏（ 契约缺失，
    ...
    //  Draft 口（additive：Draft + 可用 storyText 才展示；
    // 同文案、独立 testid；无回调时隐藏，不占位）。
    //  Draft 返回创作（spec §37 additive：Draft + 可展示会话即展示，
