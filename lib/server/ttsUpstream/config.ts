...
/**
 * 获取 Azure 语音服务的订阅密钥，兼容新旧命名。
 */
const resolveSpeechKey = (): string | undefined =>
  process.env.AZURE_SPEECH_KEY?.trim() || process.env.AZURE_TTS_KEY?.trim();

/**
 * 获取 Azure 语音服务的区域配置，兼容新旧命名。
 */
const resolveSpeechRegion = (): string | undefined =>
  process.env.AZURE_SPEECH_REGION?.trim() || process.env.AZURE_TTS_REGION?.trim();
...
const buildConfigFromEnv = (): TtsEnvConfig => {
  ...
  if (!apiKey) {
    throw new ServiceError({
      message: "缺少 AZURE_SPEECH_KEY 或 AZURE_TTS_KEY 环境变量",
      status: 500,
  ...
  if (!region) {
    throw new ServiceError({
      message: "缺少 AZURE_SPEECH_REGION 或 AZURE_TTS_REGION 环境变量",
      status: 500,
      code: "SERVER_CONFIG_ERROR",
    });
  }
