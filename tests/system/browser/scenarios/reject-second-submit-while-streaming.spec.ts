 ...
 */
test("流式中第二次提交被拒", async ({ page, harnessEnv }) => {
    ...
    /** 首个流式提示词。 */
    const firstPrompt = `流式A${Date.now()}${Math.floor(Math.random() * 100000)}请讲一个星际冒险故事。`;
    /** 流式窗口内第二次提交内容（应被互斥拒绝）。 */
    const secondPrompt = `流式B${Date.now()}${Math.floor(Math.random() * 100000)}请讲一个深海探险故事。`;
    ...
    // 中文注释：network 断言——被拒的第二次提交全程不得开新流（hardening：
