/**
 * Client-side Legacy StoryCard read compatibility boundary（）。
 *
 ...
 * 3. 语义 no-op：每个 helper 与  前 chatStore 内联分支逐字等价；
 *    任何  合法输入在  前后行为完全一致。
 */
...
/**
 * 解码/规范化历史 Legacy StoryCardPart（ B2 契约逐字迁移， relocation）。
 *
 * 铁律契约：
...
/**
 * 历史兼容卡存在性（ containment）。
 * 等价于  前 chatStore stream.intent 分支的内联判定：
...
/**
 * 故事存在性（ containment）。
 * 保持  前 isLatestMessage() 的语义：历史兼容卡存在 OR 现代
...
/**
 * 故事内容存在性（ containment）。
 * 保持  前 hasStoryMessages() 的语义：历史兼容卡 OR 现代
...
/**
 * 查找首个可播放历史兼容卡（ containment）。
 * 严格保持  前 nextStorySegment() 的行为：只寻找 Legacy 兼容卡，
