...
4. 兼容清理：
   - 删除已经没有调用方的 History 命名、测试注入和过渡适配器；
   - 删除只为旧分段播放服务、且已无生产读取的客户端状态；
   - 不在本段物理删除仍可能存在数据的数据库表。

### 最终浏览器旅程

只在本段、production build 完成后执行。必须通过可见 UI 操作，不得调用 Store、内部函数、状态机、测试 Probe 或功能开关。
...
每个视口至少留存以下可见状态截图用于本地验收，不提交运行产物：
...
浏览器旅程只在本段执行。最终报告必须分别列出：功能结果、视觉结果、未执行项、已知限制和提交列表。没有实际浏览器证据时，不得声称产品旅程已经完成。
