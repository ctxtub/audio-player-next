/**
 * StoryCollection / Conversation 切换与回退开关（change-id 2026-09-15-story-collection-continuous-creation）。
 *
 ...
 * - `LEGACY_HISTORY_READS_ENABLED`（  起默认 false）：旧 Prompt/Generation History 读路径开关；
 *   仅在显式 `'true'`/`'1'` 时打开，用于  后短期回退观测。
 * - `LEGACY_HISTORY_WRITE_ENABLED`（  起默认 false）：旧 History 新写开关；
 *    已移除 UI/store/router，本开关仅保留结构对称性，生产代码无消费者。
 *
 * 注意： 的新 Conversation/Collection/promotion 路径**结构性零 History 写入**（见 collection-domain 静态守卫），
