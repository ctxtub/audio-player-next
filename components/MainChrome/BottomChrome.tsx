...
/**
 *  BottomChrome（ §15/§16 +  spec §5/§7）。
 *
 * 职责：
 * - Mini render slot + TabBar 的统一底部容器；
 * - Mini 渲染只由 MainChrome 传入的 visible 决定（与 reservation 同源，原子一致；
 *   visible 已含  Expanded suppress：Expanded open 时 Mini presentation 隐藏）；
 * - 本组件自身不读 Session/Keyboard/viewport/UI Expanded，不 mutation 任何 store，
 *   不 clear Session、不 pause、不改 Anchor（ owner 边界）；
 * - Mini 点击语义为  useNowPlayingEntry → nowPlayingUiStore.openExpanded()
 *（URL 不变；/player 物理保留至，本层不包路由、不改 entry 契约）；
 * - 移动端 docked Mini 不可拖：本层与 Mini 均不绑定任何 drag 手势
 *（桌面 drag 归，Expanded Sheet drag 归 NowPlayingLayer/Handle）。
 */
