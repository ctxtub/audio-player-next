...
async function runWorkViewStoryNavigation(): Promise<void> {
    ...
    console.log('=== M7-04-01-I4: Draft 隐藏且无 fake-id（验收 5） ===');
    {
        resetAll();
        seedDraftSession();
        const vm = readVm();
        assert.strictEqual(vm.canViewStory, false, 'Draft 不展示');
        assert.strictEqual(vm.viewStoryTarget, null, 'Draft 不生成目标');
        const rendered = renderExpanded();
        assert.strictEqual(rendered.queryByTestId('expanded-view-story-button'), null, 'Draft 无查看正文按钮');
        assert.strictEqual(rendered.queryByTestId('expanded-actions'), null, 'Draft 无动作区占位');
        ...
        rendered.unmount();
        console.log('PASS: M7-04-01-I4 draft hidden no-fake-id');
    }
