...
| `history-reuse-create`（历史再利用创作） | 1 | L3×1 | 从提示词历史开始新创作（L2 主，当前 PLANNED 以 L3 占位，落实现改 L2）：新 Agent 上下文不含旧会话、新建干净当前会话、旧创作保留在历史可返回、新故事允许自动播放 |
...
| 语义 case_id | 中文名 | P / 主防线 / LC | 场景 spec |
| --- | --- | --- | --- |
| `rapid-double-submit-guard`（旧 E2E-02-01、H-04） | 连续快速双发防重 | P0 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/02-交互并发与竞态防御/01-快速双发防重.md) |
| `streaming-resubmit-mutex`（旧 E2E-02-02、H-02） | 流式生成中再次提交互斥 | P0 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/02-交互并发与竞态防御/02-流式生成中再次提交互斥.md) |
| `preload-continue-no-chat-pollution`（旧 E2E-02-03、H-03） | 预载续写不污染聊天流 | P0 / L1 / ACTIVE | [场景](../e2e/02-交互并发与竞态防御/03-预载续写不污染聊天流.md) |
| `clear-during-generate-no-orphan-audio`（旧 E2E-02-04、H-05） | 生成中清空防止孤儿播放 | P0 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/02-交互并发与竞态防御/04-生成中清空防止孤儿播放.md) |
...
| `failed-message-retry-context`（旧 E2E-02-10） | 失败消息重试上下文正确 | P2 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/02-交互并发与竞态防御/10-失败消息重试上下文正确性.md) |
...
| `multi-paragraph-prefetch-progress-persist`（旧 E2E-03-07） | 多段落前瞻预取推进与进度落库 | P1 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/03-播放状态与内核一致性/07-多段落前瞻预取推进与进度落库.md) |
| `story-complete-clear-progress`（旧 E2E-03-08） | 故事播完清理进度收尾 | P1 / L2 / ACTIVE | [场景](../e2e/03-播放状态与内核一致性/08-故事播完clearProgress收尾.md) |
| `client-route-nav-play-continuity`（旧 E2E-03-09） | 客户端路由导航播放连续 | P1 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/03-播放状态与内核一致性/09-客户端路由导航播放连续性.md) |
...
| 语义 case_id | 中文名 | P / 主防线 / LC | 场景 spec |
| --- | --- | --- | --- |
| `tts-voice-fallback-browser-behavior`（旧 E2E-02-12） | 语音回退浏览器行为占位 | P2 / L3 / PLANNED | [场景](../e2e/02-交互并发与竞态防御/11-播放器选历史切换当前创作.md) |
...
| `history-epoch-guard-30day-prune`（旧 E2E-04-06） | 历史纪元守卫与30天裁剪 | P2 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/04-云端存储与多端数据调和/06-历史记录epoch守卫与30天裁剪.md) |
...
> 说明：`tts-voice-fallback-browser-behavior` 的 `spec_path` 在 catalog 中指向 `02-交互并发与竞态防御/11-播放器选历史切换当前创作.md`（占位绑定），实现时随 catalog 修正，本矩阵如实转写 catalog，不自行改数。
...
| `logout-dual-cookie-clean-reset`（旧 E2E-05-04） | 登出双凭证清理与状态重置 | P0 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/05-认证授权与会话生命周期/04-登出双cookie清理与状态重置.md) |
...
| `fast-account-switch-epoch-no-crossread`（旧 E2E-05-09） | 快速账号切换纪元失效不串读 | P1 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/05-认证授权与会话生命周期/09-快速账号切换epoch失效不串读.md) |
...
| `protected-api-401-guard-matrix`（旧 E2E-06-06） | 直连受保护接口401守卫矩阵 | P1 / L2 / LEGACY-NON-COVERAGE | [场景](../e2e/06-异常处理与接口限流/06-直连受保护API之401守卫矩阵.md) |
...
| 语义 case_id | 中文名 | P / 主防线 / LC | 场景 spec |
| --- | --- | --- | --- |
| `player-history-select-switch-creation`（旧 E2E-02-11、H-21） | 播放器选历史切换当前创作 | P0 / L3 / PLANNED | [场景](../e2e/02-交互并发与竞态防御/11-播放器选历史切换当前创作.md) |

## 覆盖阅读说明

- 本矩阵只证明“绑定存在”，不证明“行为通过”；行为通过以 runner 结构化断言 + 原始证据为准。
- `LEGACY-NON-COVERAGE` 共 10 项不计产品覆盖，重写后改 lifecycle 并同步本矩阵（本矩阵由 catalog 推导，不手改总数）。
- `MANUAL` 共 1 项（`bfcache-pageshow-recover-probe`）不进自动化调度，需人工探索并留证据。
- `PLANNED` 共 25 项为预期缺口：产品语义已确定、代码尚不存在，不得为通过而降级。
